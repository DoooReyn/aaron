...
import { ExtraSpace } from '../lib';
import { Ruler } from './ruler';
import { CFG } from './config';
import { BINWriter, JSONWriter, TSWriter } from './writer';
...
function SaveSheet(
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    switch (target) {
      case 'JSON':
        Save(dirname, '.json', table, JSONWriter(data));
        break;
      case 'TS':
        {
        ...
        break;
      case 'BIN':
        Save(dirname, '.bin', table, BINWriter(data));
        break;
    }
  }
}
