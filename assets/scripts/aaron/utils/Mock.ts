        ...
        try {
          ...
          if (result instanceof Promise) {
            return result.then((data) => {
              ...
              console.log(`⏲ [${tag} ${propertyKey}] 异步耗时: ${(end - start).toFixed(3)}ms`);
              return data;
            });
          } else {
            const end = performance.now();
            console.log(`⏲ [${tag} ${propertyKey}] 同步耗时: ${(end - start).toFixed(3)}ms`);
            return result;
          }
        } catch (error) {
          console.error(`⏲ [${tag} ${propertyKey}] 发现错误:`, error);
          throw error;
        }
      };
    }
